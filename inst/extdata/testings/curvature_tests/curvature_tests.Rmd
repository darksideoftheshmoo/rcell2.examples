...
```{r}
bkp_file <- "inst/extdata/testings/curvature_tests/data/curvature_tests.RData"
...
```{r}
ids <- c("12.3.1", "12.3.2")  # Position 7, cell 71, frames 0 and 1.
...
```{r}
ggplot() + 
  geom_line(aes(x=1:b.median, y=angles[1,]), colour = "red") +
  geom_line(aes(x=1:b.median, y=angles[2,]), colour = "blue") +
  theme_minimal() + xlab("Imdex") + ylab("Angle")
```
...
ggplot(mapping=aes(x=x,y=y,color=giro3)) +
  geom_path(data = series2.dtw_centered) + 
  geom_point(data = series2.dtw_centered, size =2) + 
  geom_path(data = series1.dtw_centered, color = "gray") + 
  geom_point(data = series1.dtw_centered, shape=23, size =2) +
...
Es bastante ruidoso, repetir con suavizado:
