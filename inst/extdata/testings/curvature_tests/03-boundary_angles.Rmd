...
```{r setup, message=F}
# Install the "remotes" pacakge if missing
if(!require("remotes", quietly = T)) install.packages("remotes")

# Install "Cuentitas"
if(!require("utiles", quietly = T)) remotes::install_gitlab("naikymen-space/science/r_utils")

# Install "rcell2"
if(!require("rcell2", quietly = T)) remotes::install_github("darksideoftheshmoo/rcell2", )
...
cell.data <- rcell2.cellid::get_cell_data(data.dir)
cell.args <- rcell2.cellid::arguments(path = data.dir) |> suppressWarnings()
cell.boundaries <- 
  rcell2.cellid::get_cell_boundaries(data = cell.args, data_source = "masks.tsv", flags = 0) |> 
  unite("id.cell", pos, cellID, t.frame, remove = F)
```

```{r}
# data.dir <- system.file("extdata/testings/curvature_tests/data/", package = "rcell2.examples")
# cell.data <- readRDS(file.path(data.dir, "cell.data_20201213_60834cca08e2.RDS"))
# cell.boundaries <- readRDS(file.path(data.dir, "cell.boundaries_20201214_6083158baea5.RDS"))
```
...
id.one <- boundary.one$id.cell[1]

if(is.factor(id.one)) id.one <- as.character.factor()

ggplot(boundary.one, aes(x-mean(x),y-mean(y))) + 
...
##### Interpolate boundaries to constant length

Interpolate to the longest perimeter, or to the 90th percentile length.
...
cell.perims.smooth.list <- foreach(cell.boundary.chunk = cell.boundaries.list.chunked,
    ...
    lapply(cell.boundary.chunk, function(cell.boundary){
      ...
      xy.spline_nr <- utiles::smooth_spline_poly(xy = xy, 
...
```{r}
p <- ggplot() +
  geom_point(aes(x=x, y=y), data = cell.boundaries.list[[id.one]], color="gray") +
...
p + ggtitle(paste(
  ...
  "\nUnique perimeter lengths in dataset:",
  paste(
...
saveRDS(cell.perims.smooth.list, "/tmp/cell.perims.smooth.list.variable_length.RDS")

# saveRDS(cell.perims.smooth.list, "/tmp/cell.perims.smooth.list.p90th_length.RDS")
```
...
Sum of angles should equal 360, a full-circle.
...
```{r}
# test.id <- cell.perim.angles.list |> names() |> sample(1)
# test.id <- "6_221_0"
test.id <- id.one

ggplot(mapping = aes(x=x, y=y, color=giro3)) + 
  geom_path(color="black", data=cell.boundaries.list[[test.id]])+
  geom_path(size=2, data=cell.perim.angles.list[[test.id]]) + 
...
cell.perim.angles.list %>% saveRDS("/tmp/cell.perim.angles.list.variable_length.RDS")

cell.perim.angles.list %>% saveRDS("/tmp/cell.perim.angles.list.p90th_length.RDS")
...
```{r}
# cell.perim.angles.list <- readRDS("data/cell.perim.angles.list.RDS")
cell.perim.angles.list <- readRDS("/tmp/cell.perim.angles.list.variable_length.RDS")
...
```{r}
persistent_cells_at_t <- cell.boundaries %>% 
  select(pos, cellID, t.frame) %>% unique() %>% 
  group_by(pos, cellID) %>% filter(n() > 1)

cell.boundaries.t <- semi_join(cell.boundaries, persistent_cells_at_t)

# persistent_cells_at_t %>% filter(pos == 2, cellID == 133)
# cell.boundaries.t %>% filter(pos == 2, cellID == 133)
```

```{r}
my.sample <- persistent_cells_at_t[sample(1:nrow(persistent_cells_at_t), 1), c("pos", "cellID")]

cell.angles.one <- cell.perim.angles %>% semi_join(my.sample)

id.cell.one <- cell.angles.one[1, c("pos", "cellID")] %>% unlist() %>% paste(collapse = ".")
```

```{r}
cell.angles.one %>% ggplot(aes(x,y,color=giro3)) + 
  geom_path(size=2) + 
  facet_wrap(~t.frame, ncol = 10) +
  coord_equal() + scale_color_viridis_c() + theme_minimal() +
...
ggplot(mapping=aes(x=x,y=y,color=id)) +
  ...
  scale_color_distiller() + coord_equal() + theme_minimal() + scale_y_reverse() +
  ggtitle("Boundary index series of a cell pair", id.cell.one)

ggplot(mapping=aes(x=x,y=y,color=giro3)) +
  geom_point(data = series1) + 
  geom_point(data = series2 %>% mutate(id = recenter.vector(id, 125))) +
  scale_color_viridis_c() + coord_equal() + theme_minimal() + scale_y_reverse() +
...
ggplot(d, mapping = aes(id,giro3)) +
  geom_line() +
  geom_point(aes(color=giro3), size=2) +
...
series2.dtw_centered <- 
  ...
  mutate(id = 1:n()) |> 
  mutate(x=(x-mean(x))*1.75, y=(y-mean(y))*1.75)
...
ggplot(mapping=aes(x=x,y=y,color=id)) +
  ...
  scale_color_distiller() + theme_minimal() +
...
p <- 
  ggplot(mapping=aes(x=x, y=y, color=id)) +
    geom_segment(aes(x=x1, y=y1,
                 ...
                 data = d,
                 size=0.2, 
                 arrow = arrow(type = "closed", angle = 10, ends = "last",
                               length = unit(0.1, "in"))) +
...
ggplot(mapping=aes(x=x,y=y,color=id)) +
  geom_point(data = series2.dtw_centered) + 
  geom_point(data = series1.dtw_centered, shape=2) +
  scale_color_viridis_c() + theme_minimal() + coord_equal() + scale_y_reverse()
```
...
p <- 
  ggplot(mapping=aes(x=x,y=y,color=id)) +
    ...
    theme_minimal() + coord_equal() + scale_y_reverse()
...
```{r}
d.one <- cell.data$data %>% filter(pos == cell.angles.one$pos[1], cellID == cell.angles.one$cellID[1])

magickCell(cdata = d.one,
           paths = cell.data$images, return_single_imgs = T, cell_resize = 150) %>% 
  square_tile(nRow = 1, annot_labels = d.one$t.frame) %>% 
  magickForKnitr()
```
...
pero no de la segunda con la primera... tengo qye recalcular todo dandolos vuelta.
...
* DTW no es simétrico: las flechas que arma para A-B no son iguales a las de B-A.
...
**PROBLEMON**: los paths se cortan porque hice DTW junta todos de la primera con la segunda,
pero no de la segunda con la primera... tengo que recalcular todo dandolos vuelta.
