...
```{r}
data.dir <- "../../../data/image_samples_time_series/"

parameters.list <- rcell2.cellid::parameters_default(
  background_reject_factor = 1.3,
  max_split_over_minor = 0.6,
  max_dist_over_waist = 6,
  max_pixels_per_cell = 3500
)

cell.args <- arguments(data.dir, parameters = parameters_write(parameters.list))
```


```{r}
# cell.out <- cell.args %>%
#   # filter(ch == "TFP", t.frame %in% c(1,10,20,40)) %>%
#   cell2(output_coords_to_tsv = T)
```

## Load cell data

```{r}
cell.data <- cell.load.alt(data.dir)
cell.boundaries <- cell.load.boundaries(data.source = "masks.tsv", arguments = cell.args, flags = 0)
...
ggplot(cell.perims) +
  ...
  ggtitle("Summary values", "Difference and ratio of smooth boundary length v.s. expected length for a cirular boundary.")
```
...
```{r}
dir.create("sample_data", showWarnings = F)
cell.data %>% saveRDS(
  "sample_data/cell.data.RDS")
cell.boundaries %>% saveRDS(
  "sample_data/cell.boundaries.RDS")
cell.boundaries.list %>% saveRDS(
  "sample_data/cell.boundaries.list.RDS")
cell.perims.list %>% saveRDS(
  "sample_data/cell.perims.list.RDS")
cell.perims %>% saveRDS(
  "sample_data/cell.perims.RDS")
cell.perims.smooth %>% saveRDS(
  "sample_data/cell.perims.smooth.RDS")
cell.perim.angles %>% saveRDS(
  "sample_data/cell.perim.angles.RDS")
cell.angles.sd %>% saveRDS(
  "sample_data/cell.angles.sd.RDS")
```
...
Save sample and re-plot:

```{r}
ids <- c("2.258.13", "2.38.20")
...
plot(y2, type="l", col = "black", ylim = c(-50, 50))
...
Bueno, no parece estar tan mal, aunque lo de los ángulos es cierto, no da 360:
